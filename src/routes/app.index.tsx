    ...
    <div className="mx-auto max-w-5xl space-y-12 py-4 sm:py-8">
      ...
      <section>
        <div className="flex items-end justify-between gap-4 border-b border-border pb-4">
          <div>
            <p className="text-xs font-semibold uppercase text-muted-foreground">Continuar</p>
            <h2 className="mt-1 font-display text-2xl font-semibold">Seus projetos</h2>
          </div>
        ...
        {projects.estado === "carregando" && (
          <p className="py-6 text-sm text-muted-foreground">Carregando projetos…</p>
        )}
        {projects.estado === "erro" && (
          <p className="py-6 text-sm text-destructive">Não foi possível carregar seus projetos.</p>
        )}
        {projects.estado === "pronta" && projects.projetos.length === 0 && (
          <div className="py-8 text-center text-sm text-muted-foreground">
            <Plus className="mx-auto mb-2 size-5" />
            Seu primeiro projeto começa na conversa acima.
          </div>
        )}
        {projects.estado === "pronta" && projects.projetos.length > 0 && (
            ...
            {projects.projetos.slice(0, 4).map((project, index) => (
              ...
              >
                ...
                <span className="min-w-0">
                  <span className="block truncate text-sm font-medium">{project.nome}</span>
                  <span className="mt-0.5 block truncate text-xs text-muted-foreground">
                    {project.ideia}
                  </span>
                </span>
                <span className="flex items-center gap-1 text-xs text-muted-foreground">
                  <ShieldCheck className="size-3.5" /> Abrir
                </span>
              </Link>
            ))}
          </div>

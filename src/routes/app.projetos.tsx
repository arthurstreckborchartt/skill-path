          ...
          return {
            ...
            status:
              saved?.status === "completed"
                ? "Concluído"
                : saved?.status === "in_progress" || saved?.status === "submitted"
                  ? "Em andamento"
            ...
            progress:
              saved?.progress ??
              (step.state === "concluído" ? 100 : step.state === "atual" ? step.checkPct : 0),
              ...
              <div className="sm:w-48">
                ...
                {p.status !== "Sugerido" && (
                  ...
                  >
                    ...
                    {p.status === "Concluído" ? "Ver evidência" : "Registrar entrega"}
                  </Btn>
                )}
              </div>
            </article>
              ...
              <Panel className="w-full max-w-xl rounded-b-none sm:rounded-lg">
                ...
                <label className="mt-4 block text-sm font-medium">
                  ...
                  <textarea
                    ...
                    rows={4}
                    placeholder="Explique brevemente o que construiu e o que aprendeu."
                    className="mt-2 w-full rounded-lg border border-input bg-background p-4 outline-none focus:ring-2 focus:ring-ring"
                  />
                </label>
                <Btn
                  className="mt-5 w-full"
                  disabled={!evidenceUrl.trim() || reflection.trim().length < 10}
                  onClick={() => {
                    void learning.saveProject({
                      projectId: project.id,
                      stepId: views.find((step) => step.title === project.stepTitle)?.id ?? "",
                      title: project.title,
                      status: "completed",
                      progress: 100,
                ...
                >
                  <Check className="size-4" /> Registrar projeto concluído
                </Btn>
                <p className="mt-3 text-xs text-muted-foreground">
                  A entrega vira evidência do seu progresso. A Pathly não publica este link.
                </p>
              </Panel>
            </div>
          );

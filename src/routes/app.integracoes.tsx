...
const groups = [
  {
    ...
    body: "GitHub e GitLab podem receber tarefas, ler contexto e acompanhar mudanças com autorização da sua conta.",
    status: "Disponível primeiro",
  },
  {
    icon: Braces,
    title: "Assistentes de desenvolvimento",
    body: "Codex, Lovable, Gemini e outros entram quando oferecem uma conexão MCP ou API oficial compatível.",
    status: "Por compatibilidade",
  },
  {
    icon: Video,
    title: "Imagem, vídeo e conteúdo",
    body: "Runway, Higgsfield e serviços similares exigem integração oficial e permissões próprias antes de executar.",
    status: "Em expansão",
  },
];
  ...
  return (
    <div className="space-y-8">
      ...
      <Reveal>
        <Panel className="border-foreground/20 bg-foreground text-background">
          ...
          <h2 className="mt-4 max-w-2xl font-display text-2xl font-semibold">
            Uma conexão só aparece como pronta depois de ser validada.
          </h2>
          <p className="mt-2 max-w-2xl text-sm leading-relaxed text-background/70">
            O Pathly nunca simula uma integração e nunca executa uma ação externa sem mostrar o que
            será enviado e pedir sua confirmação.
          </p>
        </Panel>
      </Reveal>
        ...
        {groups.map((group, index) => (
          <Reveal key={group.title} delay={index * 70}>
            <Panel className="h-full">
              ...
              <Chip className="mt-5" tone="neutral">
                {group.status}
              </Chip>
            </Panel>
          </Reveal>
      ...
      <p className="text-xs leading-relaxed text-muted-foreground">
        As conexões por usuário serão liberadas gradualmente após validação dos endpoints oficiais e
        das permissões de cada fornecedor.
      </p>
    </div>
  );
}

...
export function LearningSession({ activity }: { activity: LearningActivity }) {
  ...
  async function finish() {
    ...
    const result = await learning.completeActivity({ activity, score: 100, confidence, minutesSpent });
    setEarned(result.xp);
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-5">
      ...
      <Panel className="relative min-h-[360px] overflow-hidden">
        ...
        {stage === 5 && <section className="text-center"><span className="mx-auto grid size-14 place-items-center rounded-lg bg-primary text-primary-foreground"><Check className="size-7" /></span><h2 className="mt-5 font-display text-2xl font-bold">Evidência registrada</h2><p className="mx-auto mt-3 max-w-xl text-sm leading-relaxed text-muted-foreground">Você conectou o conceito ao resultado e concluiu a prática. O domínio subiu, mas só chega a 100% após revisão e aplicação — concluir uma sessão não significa dominar.</p>{earned > 0 && <Chip tone="xp" className="mt-5"><Zap className="size-3" /> +{earned} XP por evidência</Chip>}<div className="mt-7 flex flex-col justify-center gap-3 sm:flex-row">{next ? <Link to="/app/aprender/$activityId" params={{ activityId: next.id }} className="tap inline-flex h-12 items-center justify-center gap-2 rounded-lg bg-primary px-6 text-sm font-semibold text-primary-foreground">Próxima sessão <ArrowRight className="size-4" /></Link> : <Link to="/app/rota" className="tap inline-flex h-12 items-center justify-center gap-2 rounded-lg bg-primary px-6 text-sm font-semibold text-primary-foreground">Ver meu caminho <ArrowRight className="size-4" /></Link>}</div></section>}
      </Panel>

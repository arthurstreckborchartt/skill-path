...
/*
 * Croma zero — o avatar sai em escala de cinza.
 *
 * O original usa 0.18 e deriva o matiz do nome, entao cada pessoa ganha uma cor propria. Aqui a
 * identidade e monocromatica, e o que distingue um avatar do outro continua existindo sem cor
 * nenhuma: a FORMA vem do mesmo hash, por outros uniformes (S, H, P, Q). Duas pessoas diferentes
 * continuam com avatares diferentes; os dois e que sao cinza.
 *
 * As duas luminosidades ficam como estavam (0.8 e 0.45) porque e a diferenca entre elas que
 * desenha o relevo. Aproxima-las apagaria o desenho.
 */
function getColors(hash: [number, number]): [[number, number, number], [number, number, number]] {
  const hue = deriveHue(hash);
  return [oklchToRgb(0.8, 0, hue), oklchToRgb(0.45, 0, hue)];
}
...
function computeUniforms(name: string): Uniforms {
  ...
  return {
    ...
    // `!`: o laco acima preenche exatamente oito posicoes. O `noUncheckedIndexedAccess` do
    // projeto nao consegue enxergar isso, e um `?? 0` aqui esconderia um bug de verdade se o
    // laco mudasse.
    P: [p[0]!, p[1]!, p[2]!, p[3]!],
    Q: [p[4]!, p[5]!, p[6]!, p[7]!],

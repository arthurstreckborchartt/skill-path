 ...
 *
 * 1. `next-themes` nao existe neste projeto — o tema vive em `@/lib/theme`, com `.dark` no <html>
 *    escrito por um script antes da primeira pintura. O import original quebrava o build.
 * 2. `prefers-reduced-motion` desliga o shader. O original roda `requestAnimationFrame` sem parar;
 *    e um shader de tela cheia, todo quadro, para sempre.
 * 3. Pausa com a aba escondida. Sem isso o WebGL continua desenhando atras de outra janela,
 *    gastando bateria para ninguem ver.
 */
...
export default function AnimatedGradient({
  ...
  /*
   * Quem pede menos movimento não recebe shader nenhum — nem um quadro.
   *
   * A regra global de `prefers-reduced-motion` do `styles.css` só alcança animação CSS. Isto aqui
   * é `requestAnimationFrame` desenhando WebGL, então precisa ser desligado no JavaScript.
   */
  ...
  useEffect(() => {
    ...
    if (!canvas || !container || !isMounted || semMovimento) return;
    ...
    /*
     * Aba escondida, shader parado.
     *
     * Sem isto o WebGL continua desenhando atras de outra janela — gastando bateria e GPU para
     * ninguem ver. `startTimeRef` e reajustado na volta para o gradiente retomar de onde parou em
     * vez de saltar o tempo todo que a aba ficou oculta.
     */
    let pausadoEm = 0;
    const aoTrocarVisibilidade = () => {
      if (document.hidden) {
        ...
        startTimeRef.current += performance.now() - pausadoEm;
        frameIdRef.current = requestAnimationFrame(animate);
      }
    };
...
/*
 * `parte` existe por causa do `noUncheckedIndexedAccess` do projeto: indexar um array devolve
 * `string | undefined`, e o original entregava isso direto ao `parseInt`. Um `rgb()` malformado
 * viraria `NaN` silencioso num uniform do shader — o fundo sumiria sem ninguem saber por que.
 */
function parte(partes: string[], i: number, padrao = 0): number {
...
function hexToRgba(hex: string): [number, number, number, number] {
  ...
  if (hex.startsWith("rgba(")) {
    ...
    const h = parte(parts, 0) / 360;
    const s = parte(parts, 1) / 100;
    const l = parte(parts, 2) / 100;
    a = isHsla ? parte(parts, 3, 1) : 1;

...
function progressFromJson(value: Json | null | undefined): RouteProgress | null {
  if (!value || typeof value !== "object" || Array.isArray(value)) return null;
  const candidate = value as { done?: unknown; checks?: unknown; lastActiveDate?: unknown; streak?: unknown };
...
export async function saveCloudProfile(userId: string, profile: OnboardingProfile): Promise<boolean> {
  try {

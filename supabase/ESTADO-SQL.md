...
Última verificação por sonda: **2026-09-19**, sessão autenticada em `localhost:8080`.
...
| `pathly_integracoes.sql`                     | `pathly_conexoes` e `pathly_acoes_externas` — conexões externas e o portão de aprovação                      | `executado`    | 2026-09-19, executado pelo usuário. Bateria de 14 provas em projeto descartável apagado depois: **13 passaram, 1 falhou.** Passaram — as duas tabelas respondem (antes `PGRST205`); `insert`/`update` em `pathly_conexoes` e `delete` em `pathly_acoes_externas` dão `42501 permission denied for table` (privilégio); `insert` com `user_id` alheio e em projeto que não é meu dão `42501 violates row-level security policy` (RLS, antes da FK); `pendente` → `executada` direto dá `P0001 transicao de estado invalida`; `pendente` → `aprovada` → `executada` passam; alterar `payload` depois de criada dá `P0001`; `CASCADE` ao apagar o projeto não deixou órfã. **Falhou — `select token_cifrado` como `authenticated` respondeu SEM ERRO, e deveria dar `42501`.** O `revoke` por coluna não corta privilégio de tabela (documentação do Postgres, em REVOKE). Corrigido por `pathly_integracoes_colunas_fix.sql`, ainda `gerado`. |
| `pathly_integracoes_colunas_fix.sql`         | troca o `grant select` de tabela por `grant select` de coluna em `pathly_conexoes`                           | `gerado`       | Nenhuma. Escrito em 2026-09-19 para fechar a única prova que falhou acima. **Não roda nada além de privilégio; não toca em dado.** A conferir depois de rodar, as duas juntas: `select token_cifrado` tem que dar `42501`, e `select provedor, conta, escopos` tem que responder sem erro. Se a segunda também falhar, parar e investigar.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
